...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { customerId, cartJson } = req.body;
    if (!customerId || !cartJson) {
      return res.status(400).json({ error: "customerId and cartJson are required" });
    }
    ...
    console.error("[Shopify Sync Cart Error]", error);
